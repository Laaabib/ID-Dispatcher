...
try {
  ...
  if (fs.existsSync(configPath)) {
    ...
    console.warn("firebase-applet-config.json not found, skipping specific server-side firebase initialization");
  }
} catch (e: any) {
  console.warn("Could not load firebase config in server.ts:", e.message);
}
